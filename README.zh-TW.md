...
AI 不替你做這件事。下方 skill 是 **Socratic 對話夥伴**——它用結構化的
提問逼你把原本含糊的東西講清楚；它**不會**替你想問題。
...
目前沒有原生 skill。視覺化通常就是直接跟你的繪圖工具（matplotlib /
plotly）互動。當工作是機械性（多張圖統一風格、批次重畫）或解讀性
（圖配 caption / narrative）時，請用下方 **Cross-cutting tools**——
`codex-delegate` 產繪圖腳本；`gemini-delegate` 用長 context 配
figure-caption。
